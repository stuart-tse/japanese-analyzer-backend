...
router.post('/credits/purchase', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    const payment = await asianPaymentService.createCreditPaymentWithMethod(
      customerId,
      packageIndex,
      paymentMethod
    );
...
router.post('/validate', (req: Request, res: Response) => {
  try {
    ...
    const validation = asianPaymentService.validatePaymentMethod(
      paymentMethod,
      amount,
      currency
    );
...
router.get('/pricing', (req: Request, res: Response) => {
  try {
    ...
    const creditsWithCNY = PRICING.CREDITS.map((pkg: any) => ({
      ...pkg,
      usd: {
        amount: pkg.amount,
      ...
      cny: {
        ...
        formatted: asianPaymentService.formatAmount(
          asianPaymentService.convertToCNY(pkg.amount),
          'cny'
        ),
      },
    }));
