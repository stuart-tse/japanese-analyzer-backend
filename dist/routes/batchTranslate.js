...
router.post('/', rateLimit(), async (req, res) => {
    try {
        ...
        const contentTokens = tokens.filter((t) => t.pos !== '改行' && t.pos !== '空格');
        if (contentTokens.length === 0) {
            res.json({ translations: {} });
            return;
        }
        const tokenList = contentTokens
            .map((token, index) => `${index + 1}. ${token.word} (${token.pos}${token.furigana ? `, 读音: ${token.furigana}` : ''})`)
            .join('\n');
...
//# sourceMappingURL=batchTranslate.js.map