...
if (config.googleClientId && config.googleClientSecret) {
    ...
    router.get('/google/callback', passport.authenticate('google', { session: false, failureRedirect: config.frontendUrl }), (req, res) => {
...
if (config.facebookClientId && config.facebookClientSecret) {
    ...
    router.get('/facebook/callback', passport.authenticate('facebook', { session: false, failureRedirect: config.frontendUrl }), (req, res) => {
        const { accessToken, refreshToken } = req.user;
        const params = new URLSearchParams({ accessToken, refreshToken });
        res.redirect(`${config.frontendUrl}?${params.toString()}`);
    });
...
// --- Phone OTP stub ---
router.post('/phone-login', rateLimit({ maxTokens: 5, refillRate: 0.2 }), async (req, res) => {
    try {
