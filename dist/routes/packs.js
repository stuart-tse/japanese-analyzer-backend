...
const jlptData = JSON.parse(readFileSync(resolve(__dirname, '../../data/jlpt_vocabulary_all.json'), 'utf-8'));
...
router.get('/:packId', requireAuth, async (req, res) => {
    try {
        const userId = req.jwtUser.userId;
...
router.post('/:packId/study', requireAuth, async (req, res) => {
    try {
        const userId = req.jwtUser.userId;
        ...
        const progress = await UserPackProgress.findOneAndUpdate({ userId, packId: req.params.packId }, {
...
router.post('/:packId/quiz', requireAuth, async (req, res) => {
    try {
        const userId = req.jwtUser.userId;
        ...
        await UserPackProgress.findOneAndUpdate({ userId, packId: pack.packId }, update, { upsert: true });
        ...
        if (completed) {
            ...
            for (const word of pack.words) {
                ...
                await Vocabulary.findOneAndUpdate({ userId, word }, {
                    $setOnInsert: {
                        userId, word,
                        furigana: entry?.furigana?.replace(/&nbsp;/g, ' ') || '',
                        romaji: entry?.romaji?.replace(/&nbsp;/g, ' ') || '',
            ...
            await LearningStats.findOneAndUpdate({ userId }, {
            ...
            if (currentIdx >= 0 && currentIdx < allPacks.length - 1) {
                ...
                await UserPackProgress.findOneAndUpdate({ userId, packId: nextPack.packId }, { $setOnInsert: { status: 'available', studiedWords: [], wrongWords: [], quizAttempts: 0 } }, { upsert: true });
            }
        }
        // For wrong words, update wrongCount in Vocabulary
        if (wrongWords.length > 0) {
            await Vocabulary.updateMany({ userId, word: { $in: wrongWords } }, { $inc: { wrongCount: 1 } });
        }
...
router.get('/:packId/quiz-words', requireAuth, async (req, res) => {
    try {
        const userId = req.jwtUser.userId;
        const pack = await WordPack.findOne({ packId: req.params.packId }).lean();
        if (!pack) {
