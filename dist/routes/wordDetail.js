...
const router = Router();
router.post('/', rateLimit(), geminiProxy({
    label: 'WordDetail',
    ...
    buildMessages(body) {
        const { word, pos, sentence, furigana, romaji, learningMode = 'intermediate' } = body;
        let contextWordInfo = `单词 "${word}" (词性: ${pos}`;
        if (furigana)
            contextWordInfo += `, 读音: ${furigana}`;
        if (romaji)
            contextWordInfo += `, 罗马音: ${romaji}`;
        contextWordInfo += `)`;
        let detailPrompt;
