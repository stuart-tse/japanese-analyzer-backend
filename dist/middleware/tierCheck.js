 ...
 */
export function requireFeature(featureType) {
    return async (req, res, next) => {
        try {
            const userId = req.user?.id;
            if (!userId) {
...
export async function deductCredits(req, res, next) {
    try {
        ...
        // Deduct credits
        const success = await creditService.deductCredits(userId, featureCredits.cost, featureCredits.featureType, req.params.id || req.body.id);
        if (!success) {
 ...
 */
export function checkDailyLimit(featureType) {
    return async (req, res, next) => {
        try {
            const userId = req.user?.id;
            if (!userId) {
