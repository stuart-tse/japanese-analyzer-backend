...
export class CreditService {
     ...
     */
    async deductCredits(userId, amount, featureType, relatedEntityId) {
     ...
     */
    async addCredits(userId, amount, type, description, paymentId) {
        return await prisma.$transaction(async (tx) => {
            const user = await tx.user.findUnique({
                where: { id: userId },
                select: { credits: true },
     ...
     */
    async purchaseCredits(userId, packageIndex, paymentMethodId) {
        // Get user
        ...
        const paymentIntent = await stripeService.createCreditPaymentIntent(stripeCustomerId, packageIndex);
     ...
     */
    async allocateMonthlyCredits(userId, amount, subscriptionTier) {
        return await this.addCredits(userId, amount, CreditTransactionType.MONTHLY_ALLOCATION, `Monthly credit allocation - ${subscriptionTier} tier`);
    }
    /**
     * Refund credits
     */
    async refundCredits(userId, amount, reason, relatedPaymentId) {
        return await this.addCredits(userId, amount, CreditTransactionType.REFUND, `Credit refund: ${reason}`, relatedPaymentId);
    }
    /**
     * Grant bonus credits
     */
    async grantBonusCredits(userId, amount, reason) {
        return await this.addCredits(userId, amount, CreditTransactionType.BONUS, `Bonus credits: ${reason}`);
    }
    /**
     ...
     */
    async validateFeatureCost(userId, featureType) {
        const user = await prisma.user.findUnique({
            where: { id: userId },
